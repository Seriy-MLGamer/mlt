...
static mlt_position repeat_position(mlt_properties properties,
                                    const char *name,
...
static double anim_get_angle(mlt_properties properties,
                             const char *name,
                             mlt_position position,
                             mlt_position length)
{
...
static void affine_multiply(double affine[3][3], double matrix[3][3])
{
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            output[i][j] = affine[i][0] * matrix[j][0] + affine[i][1] * matrix[j][1]
                           + affine[i][2] * matrix[j][2];
...
static void affine_max_output(
    double affine[3][3], double *w, double *h, double dz, double max_width, double max_height)
{
...
static inline void get_affine(affine_t *affine,
    ...
    if (keyed == 0) {
        ...
        double o_x
            = mlt_properties_anim_get_double(properties,
                                             "ox",
                                             repeat_position(properties, "ox", position, length),
                                             length);
        double o_y
            = mlt_properties_anim_get_double(properties,
                                             "oy",
                                             repeat_position(properties, "oy", position, length),
                                             length);
                                ...
                                int writable)
{
    ...
    *format = mlt_image_rgba;
    ...
    if (mlt_properties_get(properties, "rect")) {
        ...
        if (mlt_properties_get(properties, "rect")
            && strchr(mlt_properties_get(properties, "rect"), '%')) {
    ...
    if (scale_width != 1.0 || scale_height != 1.0) {
        ...
        mlt_properties_set(b_props,
                           "consumer.rescale",
                           mlt_properties_get(a_props, "consumer.rescale"));
        // Disable padding (resize filter)
        mlt_properties_set_int(b_props, "distort", 1);
    } else if (mlt_properties_get_int(b_props, "always_scale")
               || (!mlt_properties_get_int(b_props, "interpolation_not_required")
                   && (fill || distort || b_width > result.w || b_height > result.h
                       || mlt_properties_get_int(properties, "b_scaled")))) {
        ...
        const char *b_resource = mlt_properties_get(MLT_PRODUCER_PROPERTIES(
                                                        mlt_frame_get_original_producer(b_frame)),
                                                    "resource");
    ...
    if (*format == mlt_image_rgba && b_format == mlt_image_rgba) {
        ...
        if (scale) {
            ...
            affine_scale(desc.affine.matrix,
                         sw * MIN(geom_scale_x, geom_scale_y),
...
mlt_transition transition_affine_init(mlt_profile profile,
